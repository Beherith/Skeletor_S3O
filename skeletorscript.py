...
class S3opiece:
    def __init__(self, name, object, mesh, xoff,yoff,zoff):
        ...
        self.isafoot = False
    def __repr__(self):
        return ('S3opiece:%s parent = %s children = [%s], offsets = %s object=%s mesh=%s worldpos = %s'%(self.name, self.parent.name if self.parent is not None else None, ','.join([child.name for child in self.children]), self.loc, self.object,self.mesh, self.worldpos))
...
def parent_set(object, armature, bone): #note: doesnt work
...
def getS3ORootObject():
    ...
    for object in bpy.data.objects:
        print (object, object.parent, object.location[0], object.location[1], object.location[2])
        if object.parent is None:
            for child in bpy.data.objects:
                #print ("checking", child.parent, object)
...
class SkeletorOperator(bpy.types.Operator):
    ...
    def skeletize(context):
        ...
        #print meshes:
        for mesh in bpy.data.meshes:
            print ("Mesh",mesh, mesh.name)
        
        hierarchy = {} # a list of object:[children]
        rootobject,rootname = getS3ORootObject()
        '''
        for object in bpy.data.objects:
            print (object, object.parent, object.location[0], object.location[1], object.location[2])
        ...
        #OPTIONAL optionally: rotate the root piece to -90
        #rootobject.rotation_euler[0] = pi/2.0
        ...
        for name,piece in pieces.items():
            if piece.iktarget is not None:
                ...
                #piece.bone.constraint_add(type='IK')
                #piece.bone.constraints['IK'].target = bpy.data.objects["Armature"]
                #piece.bone.constraints['IK'].subtarget = piece.iktarget.name
            #piece.object.parent = armature_object.pose.bones[piece.name]
        ...
        '''
        #OBSOLETE AND DOESNT WORK
        #reparenting objects to bones: https://blenderartists.org/t/how-to-bone-parent-an-object-using-python/491773/2
        #some other methods that dont really work: https://blenderartists.org/t/2-82-best-method-of-parenting-a-object-to-a-bone/1213830/2
        bpy.ops.object.posemode_toggle()
        bpy.ops.object.mode_set(mode='OBJECT', toggle=False)
        for name,piece in pieces.items():
            #piece.object.parent = armature_object.pose.bones[piece.name]
            #parent_set(piece.object, armature_object, piece.name)
            #piece.object.matrix_parent_inverse = (armature_object.matrix_world @ Matrix.Translation(piece.bone.tail - piece.bone.head) @ piece.bone.matrix).inverted()
            
            bpy.ops.object.select_all(action='DESELECT')
            bpy.data.objects[name].select_set(True)
            armature_object.pose.bones[piece.name].select_set(True)
            bpy.context.scene.objects.active =  armature_object.pose.bones[piece.name]
        ...
        #bpy.context.object.pose.bones["lleg"].constraints["IK"].target = bpy.data.objects["Armature"]
        #bpy.context.object.pose.bones["lleg"].constraints["IK"].subtarget = "lleg.001"
        '''
        '''
        bpy.ops.object.editmode_toggle()
        bpy.ops.object.posemode_toggle()
        ...
        bpy.context.object.pose.bones["lleg"].constraints["IK"].target = bpy.data.objects["Armature"]
        bpy.context.object.pose.bones["lleg"].constraints["IK"].subtarget = "lleg.iktarget"
        bpy.context.object.pose.bones["lleg"].constraints["IK"].chain_count = 1
        bpy.context.object.pose.bones["lleg"].constraints["IK"].chain_count = 2
        bpy.ops.object.posemode_toggle()
        bpy.ops.object.editmode_toggle()
        bpy.ops.armature.parent_clear(type='CLEAR')
        bpy.ops.object.editmode_toggle()
        bpy.ops.object.posemode_toggle()
        bpy.context.object.pose.bones["lleg"].constraints["IK"].chain_count = 3

        '''
                    
                #bpy.ops.armature.parent_set(type='OFFSET')
                #bpy.ops.armature.parent_set(type='CONNECTED')
        #TODO: to the ends of arms, the empty flare pieces should be the IK targets
        ...
        # should we be recursing? probably :/
        
        
        #move the armature to the origin of the root piece
        
        # the endpoint of a bone:
        # if NO children : then the endpoint of a bone 
        
        #for each child:
        
        
        
        #finding feet tarantino style:
        # if it is a branch node, but very close to Y
        
        #if it is a leaf node very close to 0 Y, it is a foot. 
        #if it is a foot, then add a heel bone,
        #if it has a knee (parent of foot)
        
        # add a knee pole (left or right, respective)
        # add it to the base of the shin bone
        # move it forward, make sure the bone points forward
        
        # add a heel target to the base of the foot bone
        ...
        #constrain them to their respective bones
